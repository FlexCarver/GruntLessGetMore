module.exports = function(grunt) {
  ...
  grunt.initConfig({
        ...
        html: {
          ...
          options: {
            process: function ( content, srcpath ) {
              ...
              var theme = grunt.config.get('theme'), cnt = content.replace(/_cssframework_/gi, grunt.config.get('cssframework'));
      ...
      uglify: {
        dev: {
          ...
          files: {
            '<%= distillDir %><%= jsDir %>main.js': ['<%= jsDirTheme %>main.js', 'js/dev.js']
        ...
        prod: {
          ...
          files: {
            '<%= distillDir %><%= jsDir %>main.min.js': '<%= jsDirTheme %>main.js'
      ...
      watch: {
        ...
        js: {
            files: ['themes/<%= siteconfig.theme %>/js/*.js','js/dev.js'],
  ...
  var taskArray = [];
  if (grunt.option('prod')) {
    taskArray = ['clean:img', 'copy', 'less', 'postcss:prod', 'uglify:prod', 'clean:prod'];

  }else{
    taskArray = ['clean:dev', 'copy', 'less', 'postcss:dev', 'uglify:dev', 'watch'];
